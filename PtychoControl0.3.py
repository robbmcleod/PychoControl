...
class PtychoControl:
    ...
    def __init__( self ):
        ...
        self.defaultHexPos = 91
        self.defaultGridPos = 9
        ...
        self.zemCoilName = 'BT'
        self.zem_bt_pos = None
        
        self.ui.statusbar.showMessage( "Welcome to PtychoControl 0.3!" )
        ...
        self.ui.cbScanPatType.currentIndexChanged.connect( self.scanTypeChanged )
        self.ui.pbPlotScanPositions.clicked.connect( self.plotScanPoints )
        self.ui.rbPtychoUseBSDS.clicked.connect( self.ptychoUseBSDS )
        self.ui.rbPtychoUseIBS.clicked.connect( self.ptychoUseIBS )
        ...
        # Zemlin tableau connects
        # BT and IBT selection
        # Tilt positions is 
        self.ui.rbZemlinUseBT.clicked.connect( functools.partial( self.zemSetTiltCoil, 'BT' ) )
        self.ui.rbZemlinUseIBT.clicked.connect( functools.partial( self.zemSetTiltCoil, 'IBT' ) )
        self.ui.pbPlotZemlinTilts.clicked.connect( self.zemPlotTilts )
        self.ui.pbZemlinAcquire.clicked.connect( self.zemAcquire )
    ...
    # ZEMLIN TABLEAUS
    def zemSetTiltCoil( self, coilname ):
        self.zemCoilName = coilname
        
    def zemPlotTilts( self ):
        """
        This is just a simple piece of code to show the tilts points to the user for verification.
        """
        self.zemGenerateTilts()
        msteps = np.size( self.zem_bt_pos, 1)
        fig, ax = plt.subplots()
        ax.scatter(self.zem_bt_pos[1,:]*1E6, self.zem_bt_pos[0,:]*1E6 )
        for I in np.arange(0,msteps):
            ax.annotate(str(I), (self.zem_bt_pos[1,I]*1E6,self.zem_bt_pos[0,I]*1E6))
        plt.title( str(self.ui.cbZemlinScanPatType.currentText()) + " tilt nominal positions" )
        plt.xlabel( "X-tilt (mrad)" )
        plt.ylabel( "Y-tilt (mrad)" )
        pass
    ...
    def zemAcquire( self ):
        ...
        self.zemGenerateTilts()
        self.ui.statusbar.showMessage( "Starting Zemlin tableau acquisition" )
        msteps = np.size( self.zem_bt_pos, 1)
        init_bt = self.tem.getDeflector( self.zemCoilName )        
        
        tx = self.ui.sbZemlinTx.value()
        binning = self.ui.sbZemlinBinning.value()
        
        arrayname = self.ui.leZemlinDataName.text()
        ...
        try:
            ...
            zem_array = h5file.create_array( '/', arrayname, atom=tables.Atom.from_dtype( np.dtype(np.int32) ), shape=(msteps, self.N/binning, self.N/binning) )
        ...
        for J in np.arange(0,msteps):
            self.app.processEvents() # Unfreeze the GUI
            self.ui.statusbar.showMessage( "Acquire : " + str(J+1) + " of " + str(msteps) )
            print( " Setting beam tilt to : " + str(self.zem_bt_pos[:,J]) )
            self.tem.setDeflector( self.zemCoilName, self.zem_bt_pos[:,J] + init_bt )
            time.sleep(0.05) # settling time
            self.diffmage = self.tem.acquireImage( tx=tx, binning=binning, procmode=3 )
            self.BFDrawDiff()
            self.BFDrawFFT()
            zem_array[J,:,:] = self.diffmage
            self.ui_ptychoBF.progressBar.setValue( J )
        
        # Make sure the beam is unblanked
        self.ui.pbBeamBlank.setChecked( True )
        self.tem.setDeflector( self.zemCoilName, init_bt )
        

        # Note that in general pyQT isn't using numpy types, which is all pyTables handles.
        h5file.create_array( '/', 'mag', obj=self.tem.getImageMag()[1] )
        h5file.create_array( '/', 'tx', obj=tx )
        h5file.create_array( '/', 'binning', obj=binning)
        h5file.create_array( '/', 'scanStep', obj=self.ui.sbZemlinTiltStep.value() )
        h5file.create_array( '/', 'zem_bt_pos', obj=self.zem_bt_pos )
        h5file.create_array( '/', 'scanPattern', obj=np.str(self.ui.cbZemlinScanPatType.currentText()) )
        h5file.close()
        
        self.ui.statusbar.showMessage( "Finished Zemlin tableau" )
        pass
    ...
    def acquirePtycho( self ):
        ...
        try:
            # ptychodiff_array = fhand.create_dataset( arrayname, (self.N/self.ui.sbBinning.value(), self.N/self.ui.sbBinning.value(), msteps), chunks=(self.N/self.ui.sbBinning.value(), self.N/self.ui.sbBinning.value(), 1) )
            # So the atom type depends on the processing being used.  For now just use int32
            ptycho_array = h5file.create_array( '/', arrayname, atom=tables.Atom.from_dtype( np.dtype(np.int32) ), shape=(msteps, self.N/self.ui.sbBinning.value(), self.N/self.ui.sbBinning.value()) )
        except tables.exceptions.NodeError:
            print(  "Error could not create dataset name: " + arrayname )
            h5file.close()
            return
        ...
        for J in range(0,msteps):
            ...
            self.tem.setDeflector( self.ptychoScanCoils, self.ptycho_bs_pos[:,J] + init_bs )
            if not probeMode and self.ptychoScanCoils == 'bs':
                print( "Descanning illumination beam shift with diffraction shift." )
                self.descanBSwithDS( self.ptycho_bs_pos[:,J] )
            time.sleep(0.05) # settling time
            self.diffmage = self.tem.acquireImage( tx=tx, binning=self.ui.sbBinning.value(), procmode=3 )
            # self.BFDrawDiff()
            if self.ptychoScanCoils == 'bs':
        ...
        if self.ptychoScanCoils == "bs":
            ...
            self.tem.setDeflector( "ds", init_ds)
        elif self.ptychoScanCoils =='ibs':
            self.tem.setDeflector( "ibs", init_bs )
            pass
        
        # What other parameters would we like to save to file?  Magnification/camera 
        # length is inconsitent.  
        if( self.tem.getMode() == 'image' ):
            h5file.create_array( '/', 'mag', obj=self.tem.getImageMag()[1] )
        elif( self.tem.getMode() == 'diffraction' ):
            h5file.create_array( '/', 'camLength', obj=self.tem.getDiffractionLength()[1] )
        
        # Note that in general pyQT isn't using numpy types, which is all pyTables handles.
        h5file.create_array( '/', 'tx', obj=self.ui.sbScanStep.value() )
        h5file.create_array( '/', 'scanStep', obj=self.ui.sbPtychoTx.value() )
        h5file.create_array( '/', 'psycho_bs_pos', obj=self.ptycho_bs_pos )
        h5file.create_array( '/', 'scanPattern', obj=np.str(self.ui.cbScanPatType.currentText()) )
        h5file.close()
  
        print "Finished Ptychographic scan" 
        pass     
    
    def zemGenerateTilts( self, pattern = 'notset'  ):
        """
        Normally checks the combobox if you don't set pattern.  Otherwise you can force it use a given pattern.
        This is basically a duplicate of ptychoGenerateCoords, which I could use instead with some over-ride?
        """
        stepsize = self.ui.sbZemlinTiltStep.value() * 1e-6
        msteps = self.ui.sbZemlinTiltPos.value()
        init_bt = self.tem.getDeflector( self.zemCoilName )
        
        if pattern == 'notset':
            pattern = str(self.ui.cbZemlinScanPatType.currentText())
        
        pattern = pattern.lower()
        if pattern == 'fermat':
            # We only use xstep for the fermat spiral scan
            self.zem_bt_pos = ram.fermat_spiral( stepsize, msteps )
        elif pattern == 'hex':
            # We only use xstep for the hexagonal scan
            self.zem_bt_pos = ram.hexmesh( stepsize, msteps ) 
        else:
            print "Warning: no scan type selected"
            self.zem_bt_pos = np.array( [0.0,0.0] )
            
        # print( self.ptycho_bs_pos )
        # Add the initial position to the scans
        self.zem_bt_pos[0,:] += init_bt[0]
        self.zem_bt_pos[1,:] += init_bt[1]
        
        # Calculate the maximum
        zem_max = np.max( np.sqrt( np.sum(self.zem_bt_pos**2.0, axis=0) ) ) * 1E6
        self.ui.labelZemlinMaxTilt.setText( "%.2f"%zem_max )
    ...
    def zeroBSandDS( self ):
        ...
        self.tem.setDeflector( "ibs", np.array([0.0,0.0] ) )
...
#            self.ui.statusbar.showMessage( "WARNING: Rotation calibration not loaded from HDF5 file" )
    ...
    def ptychoUseBSDS( self ):
        self.ui.rbPtychoUseIBS.setChecked( False )
        print( "Use BS with DS descan")
        self.ptychoScanCoils = 'bs' 
        pass
    
    def ptychoUseIBS( self ):
        self.ui.rbPtychoUseBSDS.setChecked( False )
        print( "Use IBS with no descan")
        self.ptychoScanCoils = 'ibs' 
        pass
    ...
    def ptychoGenerateCoords( self, pattern = 'notset' ):
        ...
        init_bs = self.tem.getDeflector( self.ptychoScanCoils )
        ...
        if pattern == 'fermat':
            # We only use xstep for the fermat spiral scan
            self.ptycho_bs_pos = ram.fermat_spiral( stepsize, xsteps )
        elif pattern == 'hex':
            # We only use xstep for the hexagonal scan
        ...
        # print( self.ptycho_bs_pos )
        # Add the initial position to the scans
        self.ptycho_bs_pos[0,:] += init_bs[0]
        self.ptycho_bs_pos[1,:] += init_bs[1]
        
        # If the random shift checkbox is checked, add a uniform random shift of +/- 
        # the value in the spinbox
        if self.ui.cbRandomShift.isChecked():
            randlimit = self.ui.sbRandomShift.value()*1E-9
            print( "Applying random shifts of magnitude: " + str(randlimit) + " nm" )
            self.ptycho_bs_pos += np.random.uniform( low=-randlimit, high=randlimit, size=self.ptycho_bs_pos.shape)
        pass
    ...
    def ptychoCalibrateDescan( self ):
        ...
        # This is only sensible for using BS, image beam shifts are the same scan coils as the diffraction shifts
        # When using IBS you need to do the balancing in TEMspy.exe
        ...
        self.tem.setDeflector( self.ptychoScanCoils, bs_origin )
    ...
    def plotScanPoints( self ):
        """
        This is just a simple piece of code to show the scan points to the user for verification.
        """
        self.ptychoGenerateCoords()
        msteps = np.size( self.ptycho_bs_pos, 1)
        fig, ax = plt.subplots()
        ax.scatter(self.ptycho_bs_pos[1,:]*1E9, self.ptycho_bs_pos[0,:]*1E9 )
        for I in np.arange(0,msteps):
            ax.annotate(str(I), (self.ptycho_bs_pos[1,I]*1E9,self.ptycho_bs_pos[0,I]*1E9))
        plt.title( str(self.ui.cbScanPatType.currentText()) + " scan nominal positions" )
        plt.xlabel( "X-translation (nm)" )
        plt.ylabel( "Y-translation (nm)" )
    ...
    def scanTypeChanged( self ):
        ...
        if pattern == 'fermat':
            # We only use xstep for the fermat spiral scan
            self.ui.sbYScanCount.setEnabled( False )
        elif pattern == 'hex':
            # We only use xstep for the hexagonal scan
            self.ui.sbYScanCount.setEnabled( False )
        elif pattern == 'grid':
            self.ui.sbYScanCount.setEnabled( True )
    ...
    def acquirePtycho( self ):
        ...
        if self.ptychoScanCoils == "bs":
            ...
            init_bs = self.tem.getDeflector( "ibs")
            pass
    ...
    def BFDrawFFT( self, cutoff = 0.01 ):
        # Draw the FFT of the diffmage in the left window (for Zemlin tableaus)
        
        # Now repeat for diffmage
        fftmage = np.log10( np.abs( np.fft.fftshift( np.fft.fft2( self.diffmage ) ) ) )
        self.local_fft = q2np.gray2qimage( fftmage )
        self.local_fftscene = QtGui.QGraphicsScene() 
        self.pixMapItem2 = QtGui.QGraphicsPixmapItem(QtGui.QPixmap(self.local_fft), None, self.local_fftscene)
        self.ui_ptychoBF.graphicsView.setScene( self.local_fftscene )
        pass
