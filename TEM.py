...
#        import pyDM
#    else:
#        print( "Warning: TEM_FeiTitan cannot import pyDM, OS is not 32-bit" )
...
#            print( "Warning: TEM_FeiTitan cannot import pyDM, OS is not 32-bit" )
    ...
    def connectToTEM(self, tem_parent_in = None ):
        ...
        pyDM.sendMessage( "set_debuglevel_1" )
    ...
    def calcDescan(self):
        # This function is used to descan the TEM after doing something.  Generally the code to actually
        # compute the offsets is somewhere else, but here we 
        print( 'To Do' )
    ...
    def acquireImage(self, tx=1, binning=[1,1], framesize=[0,0,2048,2048], procmode=3 ):
        ...
        # unless it's proc0,1,2???
        
        if np.size( binning ) == 1:
            #print( "DEBUG: changing binning"  )
        ...
        imagecount = 1
        acqstr = "acquire_foo_"+str(imagecount)+"_" + str(tx)+"_"+str(binning[0])+"_"+str(binning[1])+"_"+str(framesize[0])+"_"+str(framesize[1])+"_"+str(framesize[2])+"_"+str(framesize[3])+"_"+str(procmode)
        # print( "Try: " + acqstr )
        ...
        while(True):
            ...
            if( pyDM.cntMsg() > 0):
                ...
                if out.split('_')[1] == "done" :
                    if( procmode == 1):
                        bytedepth = 2
                        mage = pyDM.getImage( (framesize[2]-framesize[0])/binning[0], (framesize[3]-framesize[1])/binning[1], bytedepth) #bytedepth = 2
                        
                    elif( procmode == 2 or procmode == 3):
                        bytedepth = 4
                        # print "Debug: FIXME, crash on returning processed images due to bytedepth issues" 
                        mage = pyDM.getImage( (framesize[2]-framesize[0])/binning[0], (framesize[3]-framesize[1])/binning[1], bytedepth) #bytedepth = 4
                    break
                elif out.split('_')[1] == "error" :
                    print( "Error in AcquireImage" )
                    return
                else:
    ...
    def acquireImage_FEISafe( self, tx=1, binning=1, framesize=0, proc=1 ):
        # Remember that Python uses whatever CCD is selected within the TEM control software
        # REFERENCE MODE
        # AcqImageCorrection_Default    =1          # from enum AcqImageCorrection
        # AcqImageCorrection_Unprocessed=0          # from enum AcqImageCorrection
        ...
        # Saving to RAMdisk and then re-reading a 16-bit TIFF is much faster, about 0.3 s
        # 0 is tag for a 16-bit TIFF
        return self.titan.Acquisition.AcquireImages().Item(index=0).AsSafeArray
    ...
    def getStigmator( self, stigname ):
        stigname = stigname.lower()
        # I use cstig, ostig, and dstig so we don't have a repeated string for diffraction shift
        if( stigname == 'cstig' ):
            return self.titan.Illumination.CondenserStigmator
        elif( stigname == 'ostig' ):
            return self.titan.Projection.ObjectiveStigmator
        elif( stigname ==  'dstig' ):
            return self.titan.Projection.DiffractionStigmator
            
    def setStigmator( self, stigname, stigvalue ):
        stigname = stigname.lower()
        
        # I use cstig, ostig, and dstig so we don't have a repeated string for diffraction shift
        if( stigname == 'cstig' ):
            self.titan.Illumination.CondenserStigmator = stigvalue
        elif( stigname == 'ostig' ):
            self.titan.Projection.ObjectiveStigmator = stigvalue
        elif( stigname ==  'dstig' ):
            self.titan.Projection.DiffractionStigmator = stigvalue
    ...
    def setBeamBlanker( self, status=False):
        self.titan.Illumination.BeamBlanked = bool(status)
        
    def getBeamBlanker( self ):
        return self.titan.Illumination.BeamBlanked 
...
class TEM_Simulator:
    ...
    def acquireImage(self, tx=1, binning=[1,1], framesize=[0,0,2048,2048], procmode=1 ):
        # return a testbar image
        # 
        try: 
            mage = self.iterateHDF5()
        except IOError:
    ...
    def getStigmator( self, stigname ):
        ...
        # I use cstig, ostig, and dstig so we don't have a repeated string for diffraction shift
        if( stigname == 'cstig' ):
            return self.sim_cstig
        elif( stigname == 'ostig' ):
            return self.sim_ostig
        elif( stigname ==  'dstig' ):
            return self.sim_dstig
    ...
    def setStigmator( self, stigname, stigvalue ):
        ...
        # I use cstig, ostig, and dstig so we don't have a repeated string for diffraction shift
        if( stigname == 'cstig' ):
            self.sim_cstig = stigvalue
        elif( stigname == 'ostig' ):
            self.sim_ostig = stigvalue
        elif( stigname ==  'dstig' ):
            self.sim_dstig = stigvalue
    ...
    def getStage( self, stagename='all' ):
        ...
        if( stagename ==  'all' ):  
            ...
            return self.sim_stage[0:2] # Remember indicing doesn't include the last digit by default...
        elif( stagename == 'xyz' ):
            return self.sim_stage[0:3]
        elif( stagename == 'alpha' ):
    ...
    def setBeamBlanker( self, status=False):
        self.sim_BeamBlanked = bool(status)
        
    def getBeamBlanker( self ):
        return self.sim_BeamBlanked
